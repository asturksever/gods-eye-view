...
export const CREDENTIALS = Object.freeze([
  ...
  {
    name: 'MAPILLARY_CLIENT_TOKEN',
    label: 'Mapillary client token',
    keychain: [
      ['mapillary', 'client-token'],
      ['mapillary', 'token'],
    ],
  },
  {
    name: 'ANTHROPIC_API_KEY',
    label: 'Anthropic (Mapillary AI planner)',
...
export function inspectSetup({
  ...
  return {
    ready:
      node.level !== 'error' && npmResult.status === 0 && dependenciesInstalled,
...
export function formatSetupReport(report, { readyMessage } = {}) {
  const hasKeychainSource = Object.values(report.credentials || {}).some(
    (credential) => credential?.source === 'macOS Keychain',
  );
  ...
  const lines = [
    ...
    report.npm.available
      ? `[OK] npm ${report.npm.version}`
      : '[ERROR] npm was not found',
    report.dependenciesInstalled
      ? '[OK] dependencies installed'
      : '[WARN] dependencies missing; run npm install',
    '',
