...
Turn on **Street Level** and Mapillary's coverage drapes over the streets in green (magenta for 360° captures, dimmer for imagery older than two years). Click a sequence and its image cones appear; click a cone, or press **LOOK HERE**, and the embedded MapillaryJS viewer opens inside the Mapillary dock. **STREET COCKPIT** locks the globe camera to the street-level pose, so on a photorealistic city you see the photograph and the 3D mesh of the same corner side by side.

**Mapillary AI** is a typed query box. _"show me all fire hydrants in Sacramento"_ becomes a plan (place, taxonomy classes, time window), the place is geocoded, and the city is fanned out over z14 map-feature tiles that stream back as NDJSON — the hydrants fill in tile by tile from the centre outwards, first as dots, then as Mapillary's own icons once the run completes. Follow-ups refine the previous plan: _"only the ones seen after 2024"_, _"now stop signs there"_, _"same thing in Detroit"_. The planner runs on Claude with a prompt-cached taxonomy of 43 object classes and 900 traffic-sign families; unknown classes are reported rather than guessed. Works for any place Nominatim can find, capped at 600 tiles (roughly a large city) per run.
