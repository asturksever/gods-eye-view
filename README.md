...
Sixteen layers and map sources. **Thirteen have a keyless path.** Some offer additional capabilities with a provider key. (🟢 no key · 🟡 free key · 🔴 metered.)
...
| 🟡  | **Mapillary**   | 📷 Street-level coverage, sequences and the embedded viewer, plus the detected objects + traffic signs behind the plain-English query box. A client token runs in the browser by design (MapillaryJS fetches imagery directly) | [mapillary.com/dashboard/developers](https://www.mapillary.com/dashboard/developers) — register an app, copy the client token                                        |
| 🔴  | **Anthropic**   | 🧠 the plain-English query box — turns a sentence into a Mapillary query plan (Claude, low effort, prompt-cached; a few cents per hundred questions). Server-side only                                                         | [console.anthropic.com](https://console.anthropic.com/settings/keys) — metered                                                                                       |
