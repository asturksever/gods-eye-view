...
| 📷 **Street Level**         | Mapillary coverage drawn onto the streets from orbit down to a single road, image cones per sequence, an embedded MapillaryJS viewer with a **CAMERA FOLLOWS VIEW** mode, and a plain-English query box (_"show me all fire hydrants in Sacramento"_) that streams Mapillary's detected objects and traffic signs onto the globe with Mapillary's own icons                                                        | Mapillary (+ Anthropic for the query box) | 🟡 Mapillary client token · 🔴 Anthropic key for plain-English queries only                         |
...
| 🟡  | **Mapillary**   | 📷 Street-level coverage, photos and detected objects on the globe. A public client token by design (see SECURITY.md)                                                                | [mapillary.com/dashboard/developers](https://www.mapillary.com/dashboard/developers) — register an app, copy its Client Token                                        |
| 🔴  | **Anthropic**   | 💬 Plain-English questions in the Street Level panel, turned into Mapillary queries server-side. Coverage, photos and filters work without it                                        | [console.anthropic.com](https://console.anthropic.com/settings/keys) — metered, roughly a cent per question                                                          |
